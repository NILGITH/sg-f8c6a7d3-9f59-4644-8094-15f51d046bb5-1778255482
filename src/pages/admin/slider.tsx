...
import { Plus, Edit, Trash2, Save, Upload, X, Image as ImageIcon, MoveUp, MoveDown } from "lucide-react";
...
export default function AdminSlider() {
  ...
  const [editingSlide, setEditingSlide] = useState<Partial<Slider> | null>(null);
  ...
  const sortedSlides = [...slides].sort((a, b) => (a.order_position || 0) - (b.order_position || 0));

  const handleAddSlide = () => {
    const maxOrder = slides.length > 0 ? Math.max(...slides.map(s => s.order_position || 0)) : 0;
    setEditingSlide({
      title: "",
      subtitle: "",
      description: "",
  ...
  const handleSaveSlide = async () => {
    ...
    try {
      if (isAdding) {
        await sliderService.create(editingSlide as Omit<Slider, "id" | "created_at">);
      } else if (editingSlide.id) {
        await sliderService.update(editingSlide.id, editingSlide);
      }
      await loadSlides();
  ...
  const handleToggleActive = async (slide: Slider) => {
    try {
      await sliderService.update(slide.id, { is_active: !slide.is_active });
      await loadSlides();
    } catch (error) {
  ...
  const handleMoveUp = async (slide: Slider) => {
    const currentIndex = sortedSlides.findIndex(s => s.id === slide.id);
    if (currentIndex > 0) {
      const prevSlide = sortedSlides[currentIndex - 1];
      try {
        await sliderService.update(slide.id, { order_position: prevSlide.order_position });
        await sliderService.update(prevSlide.id, { order_position: slide.order_position });
        await loadSlides();
      } catch (error) {
  ...
  const handleMoveDown = async (slide: Slider) => {
    const currentIndex = sortedSlides.findIndex(s => s.id === slide.id);
    if (currentIndex < sortedSlides.length - 1) {
      const nextSlide = sortedSlides[currentIndex + 1];
      try {
        await sliderService.update(slide.id, { order_position: nextSlide.order_position });
        await sliderService.update(nextSlide.id, { order_position: slide.order_position });
        await loadSlides();
      } catch (error) {
    ...
    <>
      ...
      <AdminLayout>
        <div className="space-y-8">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div className="ml-8">
                  <p className="text-2xl font-bold">{slides.filter(s => s.is_active).length}</p>
              ...
              <CardContent className="space-y-6">
                  ...
                  {previewUrl ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" onClick={() => { setPreviewUrl(""); if (editingSlide) setEditingSlide({ ...editingSlide, image: "" }); }}>
                          <X className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={editingSlide.cta_text || ""} onChange={(e) => setEditingSlide({ ...editingSlide, cta_text: e.target.value })} placeholder="Ex: En savoir plus" />
                  </div>
                  <div>
                    <label className="text-sm font-medium">Lien du bouton</label>
                    <Input value={editingSlide.cta_link || ""} onChange={(e) => setEditingSlide({ ...editingSlide, cta_link: e.target.value })} placeholder="Ex: #editions ou /contact" />
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <input type="checkbox" id="isActive" checked={editingSlide.is_active} onChange={(e) => setEditingSlide({ ...editingSlide, is_active: e.target.checked })} className="w-4 h-4" />
                  <label htmlFor="isActive" className="text-sm font-medium">Slide actif (visible sur le site)</label>
                </div>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-foreground">Slides ({sortedSlides.length})</h3>

            {sortedSlides.length === 0 ? (
              <Card><CardContent className="text-center py-8 text-foreground/50"><p className="text-sm">Aucun slide configuré</p></CardContent></Card>
            ) : (
              <div className="space-y-4">
                        ...
                        <div className="flex-1 space-y-2">
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex gap-1">
                              <Button variant="outline" size="sm" onClick={() => handleMoveUp(slide)} disabled={index === 0}><MoveUp className="w-4 h-4" /></Button>
                              <Button variant="outline" size="sm" onClick={() => handleMoveDown(slide)} disabled={index === sortedSlides.length - 1}><MoveDown className="w-4 h-4" /></Button>
                            </div>
                          </div>
                          {slide.description && <p className="text-sm text-foreground/60 line-clamp-2">{slide.description}</p>}
                          <div className="flex items-center gap-2 text-xs text-foreground/60">
                            <span>CTA: {slide.cta_text}</span><span>•</span><span>Lien: {slide.cta_link}</span>
                          </div>
                        </div>
